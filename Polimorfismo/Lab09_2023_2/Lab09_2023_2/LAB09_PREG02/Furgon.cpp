...
void Furgon::imprime(ofstream& arch) {
    ...
    if (pdeposito.empty()) arch << "No hay pedidos para el cliente"<<endl;
        ...
        for (NPedido ped: pdeposito)
            ped.imprimePedido(arch);
    }
}

void Furgon::cargadeposito(ifstream& arch, class NPedido& ped) {
    Vehiculo::cargadeposito(arch, ped);
    double posibleCarga = ped.GetPeso() + GetActcarga();
    if (posibleCarga <= GetMaxcarga()){
        pdeposito.push_front(ped); // comportamiento de pila
